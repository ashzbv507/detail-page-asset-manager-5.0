...
export default function ShareViewer({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch(`/api/shares/${encodeURIComponent(token)}`).then(async (response) => {
      ...
      setCollapsedGroups(new Set(sharedTasks.map((task) => task.product)));
