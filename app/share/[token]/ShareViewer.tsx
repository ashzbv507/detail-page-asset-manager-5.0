...
import { ArrowLeftRight, ChevronDown, ChevronRight, Copy, ExternalLink, Share2, X } from "lucide-react";
import { generateGeneralHtml } from "../../lib/html";

type SharedImage = { id: string; name: string; url: string; excludeFromKurly?: boolean };
type SharedTask = { id: string; brandKey?: string; product: string; item: string; html?: string; storeLink?: string; vendors?: string[]; note?: string; thumbnailNas?: string; detailNas?: string; images?: SharedImage[] };
...
function SharedCopyCell({ value, label }: { value: string; label: string }) {
  return <div className="cell-copy"><span title={value}>{value || "-"}</span>{value && <button type="button" aria-label={`${label} 복사`} onClick={(event) => { event.stopPropagation(); void copyText(value); }}><Copy size={16} /></button>}</div>;
}
...
function SharedDetailPanel({ task, onClose }: { task: SharedTask; onClose: () => void }) {
  ...
  const html = htmlPanelMode === "general" ? (task.html || generateGeneralHtml(activeImages)) : generateGeneralHtml(activeImages);
  ...
  return <aside className="detail-panel saved-detail-panel share-detail-panel">
    ...
    <div className="detail-body">
      ...
      <section className="detail-section paths"><h3>NAS 경로</h3><SharedPathRow label="썸네일" value={task.thumbnailNas ?? ""} /><SharedPathRow label="상세페이지" value={task.detailNas ?? ""} /></section>
    </div>
  </aside>;
}
...
export default function ShareViewer({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    }).then((payload) => setTasks(payload.share?.tasks ?? [])).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "공유 정보를 불러오지 못했습니다.")).finally(() => setLoading(false));
  }, [token]);
  const groups = useMemo(() => Object.entries(tasks.reduce<Record<string, SharedTask[]>>((all, task) => { (all[task.product] ??= []).push(task); return all; }, {})), [tasks]);
  ...
  return <main className="share-page"><header className="app-header share-app-header"><div className="brand-heading"><div className="brand-switcher share-brand-switcher"><img src={activeBrandImage} alt="" /></div><div className="app-title"><h1>Detail Page Asset Manager</h1><p>읽기 전용 공유 링크 · 편집할 수 없습니다.</p></div></div><div className="actions"><button type="button" className="share-header-copy" onClick={() => void copyText(window.location.href)}><Copy size={16} /> 링크 복사</button></div></header><div className={`workspace share-workspace ${selectedTask ? "with-detail" : ""}`}><section className="table-shell"><div className="table-scroll"><table className="share-table"><colgroup><col className="c-name"/><col className="c-type"/><col className="c-link"/><col className="c-html"/><col className="c-nas"/><col className="c-nas"/><col className="c-note"/></colgroup><thead><tr><th>제품명</th><th>품목</th><th>링크</th><th>HTML / URL</th><th>썸네일 NAS</th><th>상세페이지 NAS</th><th>참고사항</th></tr></thead><tbody>{loading ? <tr><td colSpan={7}><div className="share-empty">공유 정보를 불러오는 중입니다.</div></td></tr> : error ? <tr><td colSpan={7}><div className="share-empty error">{error}</div></td></tr> : groups.length === 0 ? <tr><td colSpan={7}><div className="share-empty">공유된 자산이 없습니다.</div></td></tr> : groups.map(([product, grouped], index) => <Fragment key={product}><tr className={`product-row tone-${index % 2}`} onClick={() => toggleGroup(product)}><td><button className="expand" type="button" aria-label={`${product} 하위 품목 ${collapsedGroups.has(product) ? "펼치기" : "접기"}`} onClick={(event) => { event.stopPropagation(); toggleGroup(product); }}>{collapsedGroups.has(product) ? <ChevronRight size={16} /> : <ChevronDown size={16} />}</button><b>{product}</b></td><td><span className="count">{grouped.length}개</span></td><td /><td /><td /><td /><td /></tr>{!collapsedGroups.has(product) && grouped.map((task) => <tr className="item-row" key={task.id} onClick={() => setSelectedTask(task)}><td>{task.product}</td><td>{task.item}</td><td>{task.storeLink ? <a href={task.storeLink} target="_blank" rel="noopener noreferrer" onClick={(event) => event.stopPropagation()}>열기</a> : "-"}</td><td><div onClick={(event) => event.stopPropagation()}><SharedCopyCell value={task.html ?? ""} label={`${task.product} ${task.item} HTML`} /></div></td><td><div onClick={(event) => event.stopPropagation()}><SharedCopyCell value={task.thumbnailNas ?? ""} label="썸네일 NAS" /></div></td><td><div onClick={(event) => event.stopPropagation()}><SharedCopyCell value={task.detailNas ?? ""} label="상세페이지 NAS" /></div></td><td>{task.note || "-"}</td></tr>)}</Fragment>)}</tbody></table></div><footer><span>제품 {groups.length}개 · 품목 {tasks.length}개</span><span className="share-readonly-label"><Share2 size={14} /> 읽기 전용</span></footer></section>{selectedTask && <SharedDetailPanel task={selectedTask} onClose={() => setSelectedTask(null)} />}</div></main>;
}
